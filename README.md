...
- 首页按“财务数据”“合同管理”等业务事项进入，不显示数据库表名、字段名或 SQL
- 已内置 6 项真实业务查询：认款明细、认款汇总、报销发票、合同金额汇总、合同金额明细、印花税明细
- 用户按业务语言选择年度、月份或日期等已批准条件，并选择自己要查看和导出的中文字段
- 后台受控执行已批准的多表关联查询；用户不需要理解 `INNER JOIN`、表结构或关联规则
- 可保存个人常用查询（只保存名称、条件和字段选择，不保存密码、导出结果或业务数据）
...
页面会显示“模拟数据模式”提示；可按部门、年龄范围筛选模拟员工，选择展示字段、预览结果、保存常用查询并下载 Excel。模拟数据不会访问网络或 SQL Server。

真实 SQL Server 模式会运行代码中版本管理的已批准业务查询。员工测试数据会额外读取 `dbo.employees` 的字段元数据及最多 200 个部门候选值；其他业务查询只展示固定的年度、月份、日期条件。它不会根据数据库任意字段自动开放敏感字段，也不会执行写入操作。
...
- 正常、无数据、筛选条件非法、连接失败、大结果集、常用查询保存/恢复等场景均已测试。
- 每次查询均写入审计日志；常用查询和审计日志均不含数据库密码或结果明细。
